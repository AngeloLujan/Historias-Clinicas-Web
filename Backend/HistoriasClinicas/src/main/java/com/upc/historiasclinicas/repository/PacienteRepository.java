...
public interface PacienteRepository extends JpaRepository<Paciente,Integer> {
    ...
    @Query("SELECT p FROM Paciente p WHERE (:nombres = ''  or p.nombres = :nombres) AND (:apellidoPaterno = ''  or p.apellidoPaterno = :apellidoPaterno) AND (:apellidoMaterno = ''  or p.apellidoMaterno = :apellidoMaterno) AND (:tipoDocumento = '' or :tipoDocumento = '-1'  or p.tipoDocumento = :tipoDocumento) AND (:numeroDocumento = ''  or p.numeroDocumento = :numeroDocumento)")
